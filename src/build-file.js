...
const compose_data = require('../static/compose-data.json');

const ph = "~~~~~~~~";
var quantities = {};

module.exports = function (services, serviceInfo) {
...
const getComponentValue = function (propName) {
  ...
  } else if (compose_data.listOfOptions[propName] !== undefined) {

    return compose_data.listOfOptions[propName].join('|');
...
const fillQuantities = function (service) {
  ...
  compose_data.serviceProps.quant.forEach((quantComponent) => {
    if (service['components'].indexOf(quantComponent) !== -1) {
      quantities[quantComponent] = parseInt(service[`${quantComponent}-quantity`]);
    }
  });
  compose_data.serviceProps.build.quant.forEach((buildQuantComponent) => {
